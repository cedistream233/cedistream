...
(async () => {
  ...
  try {
    ...
    console.log('Connected to database. Running add_songs migration...');
    await client.query(sql);
    console.log('add_songs migration applied successfully.');
  } catch (err) {
    console.error('add_songs migration failed:', err);
    process.exitCode = 1;
  } finally {
    await client.end();
  }
